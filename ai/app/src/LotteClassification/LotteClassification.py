...
# from src.LotteClassification import initialize
from app.src.LotteClassification import initialize
    ...
    def _inference(self, data_loader, individually=False):
        ...
        with torch.no_grad():
            for data in tqdm(data_loader):
                ...
                losses_m.update(loss.item(), input_data.size(0))
                top1_m.update(acc1.item(), output.size(0))
                top5_m.update(acc5.item(), output.size(0))
